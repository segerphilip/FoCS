...
Remarks, if any: took way longer because I forgot how if works
...
let neg_int = ("neg_int","/n.pair (not (fst n)) (snd n)")  (*finally works for simplify q2_defs "fst (neg_int (neg_int (int _3)))";;*)

(* let plus_int_ATTEMPT1 = ("plus_int","/m./n.
  if (and (fst m) (fst n)) 
  (
  ...
  if (and (fst m) (geq (snd n) (snd m))) 
  (
    pair (false) (minus (snd n) (snd m))
  )
  if (and (fst n) (geq (snd n) (snd m))) 
  (
    pair (true) (minus (snd n) (snd m))
  )
  if (and (fst n) (geq (snd m) (snd n))) 
  (
    pair (false) (minus (snd m) (snd n))
  )
  if (and (not (fst n)) (not (fst m))) 
  (
    pair (false) (plus (snd m) (snd n))
  )")  *)(* Ifs don't work like this, but keeping because this was an attempt, rewrite below *)
...
(
  ...
  (
    pair (false) (plus (snd m) (plus n))
  )
)") (* So it doesn't seem to work with: simplify q2_defs "snd (plus_int (neg_int (int _3)) (neg_int (int _2)))";;
        but otherwise works for everything else, better than before the rewrite by miles *)
...
let q3_defs = default_defs @ q2_defs @ q1_defs @ [empty; cons; match_list; length; sum; append; map]
